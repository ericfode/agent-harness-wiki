...
## Feedback-rich gyms
[[on-policy-self-distillation]] suggests a stronger requirement for training-grade environments: they should emit feedback that can be replayed and distilled, not only a reward number. Compiler errors, runtime exceptions, failed unit tests, counterexamples, rubric rationales, and user follow-ups can all become learning material if they are preserved with enough structure.

This matters most on hard tasks where pass@K is initially very low. The self-distillation interview uses `discovery@K` for hard coding tasks and argues that directionally useful failed-test feedback can teach before the first complete solution appears. That is a different substrate from a benchmark that only tells the agent it lost.
